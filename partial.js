...
!(function (G) {
  ...
  function merge_args(args1, args2, args3) {
    if (!args2.length)
      return args3
        ? _to_unde(args1, args3)
        : _to_unde(_.clone(args1));
    ...
    if (!args3)
      return _to_unde(n_args1, args2.length ? args2 : undefined);
  ...
  each(
    ...
    function (name) {
      _["is_" + name.toLowerCase()] = _["is" + name] = function (
        obj
      ) {
        return (
      ...
      try {
        if (v) {
          if (v.__mr) {
            ...
            if (need_catch ? !f.__catch_pipe : f.__catch_pipe)
              continue;
            if (v.__stop)
              return v.length == 1 ? v[0] : (v.__stop = false || v);
            v = f.apply(undefined, v);
            continue;
          } else if (v.then && _.is_fn(v.then))
            return go_async(null, v, fs, i);
        }
  ...
  function unpack_promise(v) {
    return is_mr(v)
      ? thenable_mr(v) &&
          _.go(
            Promise.all(PA == Promise.all ? v : _.toArray(v)),
            to_mr
          )
      : thenable(v) && v;
  }
    ...
    return new Promise(function (rs, rj) {
      ...
      (function go(v) {
        do {
          ...
          if (
            (need_catch ? !fs[i].__catch_pipe : fs[i].__catch_pipe) &&
            i++
          )
            continue;
          need_catch = false;
  ...
  _.all2 = function (args) {
    ...
    while (++i < al) {
      tmp = arguments[i][_.is_mr(args) ? "apply" : "call"](
        thiz,
        args
      );
      if (_.is_mr(tmp))
  ...
  _.all = _.All = function (arg) {
    ...
    if (_.is_array(fns))
      return _.all2.apply(this, [to_mr(arg)].concat(fns));
  ...
  _.spread2 = function (args) {
    var fns = _.rest(arguments, 1),
      ...
      tmp;
    for (
      var i = 0, fl = fns.length, al = args.length;
      i < fl || i < al;
      i++
    ) {
      tmp = _.is_mr(args[i])
  ...
  _.spread = _.Spread = function () {
    ...
    if (_.isArray(fns))
      return _.spread2.apply(
        this,
        [to_mr(_.initial(arguments))].concat(fns)
      );
  ...
  _.if = _.If = function (predi, fn) {
    ...
    G.console &&
      console.warn("_.if 함수는 곧 _.if2로 대체될 예정입니다.");
    return _.extend(If, {
      else_if: elseIf,
      elseIf: elseIf,
      else: function (fn) {
    ...
    return function () {
      ...
      return _.extend(If, {
        ...
        elseIf: elseIf,
          ...
          return (
            store.push([_.constant(true), _.pipe(arguments)]) && If
          );
        },
      });
      function elseIf() {
  ...
  _.f = function (nodes) {
    ...
    return (
      ...
      (setTimeout(function () {
        ...
        function () {
          return (f || (f = _.val(window, nodes))).apply(
            this,
            arguments
          );
        })
  ...
  _.is_empty = _.isEmpty = function (v) {
    return (
      (likearr(v) &&
      (_.isArray(v) || _.isString(v) || _.isArguments(v))
        ? v
        : _keys(v)
      ).length === 0
    );
    ...
    _.matcher =
      function f(object, attrs) {
        ...
        for (var i = 0, obj = Object(object), key; i < length; i++)
          if (attrs[(key = keys[i])] != obj[key] || !(key in obj))
            return false;
        return true;
      };
  ...
  function collf(fn, asy_fn) {
    return function f(d, i) {
      ...
      if (asy_fn && l)
        mp = iter(data[(k = ks ? ks[++i] : ++i)], k, data);

      return (
        (mp &&
        (mp.__mr ? thenable_mr(mp) : mp.then && _.is_fn(mp.then))
          ? asy_fn
          : fn) || asy_fn
  ...
  _.filter = collf(
    function (data, iter, ks, evd, i, l, k) {
      ...
      var res = evd ? [data[k]] : [];
      while (++i < l)
        if (iter(data[(k = ks ? ks[i] : i)], k, data))
          res.push(data[k]);
      return res;
    },
    function f(data, iter, ks, mp, i, l, k, res) {
      ...
      function (mem, val) {
        ...
        return mem.concat(
          i >= len
            ...
            ? f(
                current,
                new_keys || (new_keys = _.rest(keys, i).join("."))
              )
            : void 0
        );
      },
  ...
  // async not supported
  _.groupBy = _.group_by = collf(function (
    data,
    iter,
    ks,
    evd,
    ...
    while (++i < l)
      _.has(
        res,
        (evd = iter((val = data[(k = ks ? ks[i] : i)]), k, data))
      )
        ? res[evd].push(val)
        : (res[evd] = [val]);
    return res;
  ...
  // async not supported
  _.indexBy = _.index_by = collf(function (
    data,
    iter,
    ks,
    evd,
  ...
  // async not supported
  _.countBy = _.count_by = collf(function (
    data,
    iter,
    ks,
    evd,
  ...
  _.first =
    ...
    _.take =
      function f(ary, n, guard) {
        if (arguments.length == 1 && _.isNumber(ary))
          return _1(f, _, ary);
  ...
  // async not supported
  _.findLastIndex = _.find_last_i = collf(function (
    data,
    iter,
    X,
    X,
  ...
  _.index_of = _.indexOf = createIndexFinder(
    1,
    _.findIndex,
    _.sortedIndex
  );
  _.last_index_of = _.lastIndexOf = createIndexFinder(
    -1,
    _.findLastIndex
  );
  ...
  // async not supported
  _.mapObject = _.map_object = collf(function (
    data,
    iter,
    ks,
    X,
  ...
  _.pick = function f(d, i) {
    ...
    if (arguments.length == 2)
      ...
      var data = arguments[arguments.length - 2],
        iter = Iter(arguments);
    if (!data) return {};
    if (this != _ && this != G && _.is_fn(iter))
      iter = _.bind(iter, this);
    var res = {},
      i = -1;
    if (_.is_fn(iter)) {
      var keys = _keys(data),
  ...
  _.omit = function f(d, i) {
    ...
    if (_.is_fn(iter)) {
      var keys = _keys(data),
        l = keys.length;
      while (++i < l)
        if (!iter(data[keys[i]], keys[i], data))
      ...
      var oKeys = _keys(data),
        ...
        l = oKeys.length;
      while (++i < l)
        if (keys.indexOf(oKeys[i]) == -1)
          res[oKeys[i]] = data[oKeys[i]];
    }
    return res;
  };
  ...
  _.template = _.t = function () {
    return s.apply(
      null,
      [convert_to_html].concat(_.toArray(arguments))
    );
  };
  _.template$ = _.t$ = function () {
    return s.apply(
    ...
    return map(source.match(re), function (matched) {
      try {
        return new Function(
          ...
          "return " +
            matched.substring(length, matched.length - length) +
            ";"
        );
      } catch (e) {
        console.log(e);
  ...
  function convert_to_html(source) {
    ...
    for (var i = 0; i < ary.length; i++) {
      ...
      if (!is_paragraph) {
        ...
        if (tmp.match(REG5))
          is_paragraph = number_of_tab(RegExp.$1) + 1;
        continue;
      }
      ary[i] = ary[i].replace(
        REG6[is_paragraph] ||
    ...
    while (tag_stack.length)
      ary[ary.length - 1] += end_tag(tag_stack.pop()); // 마지막 태그
  ...
  function start_tag(str, tag_stack, attrs, name, cls) {
    ...
    // name
    name =
      !name || name == "d" ? "div" : name == "sp" ? "span" : name;
  ...
  _.extend(_, {
    ...
    extend2: function (start, sel /*, objs*/) {
      _.extend.apply(
        null,
        [_.sel(start, sel)].concat(_.rest(arguments, 2))
      );
      return start;
    },
  ...
  _.immutable = _.im = _.extend(
              ...
              ? /*start*/ !key.match(/\[(.*)\]/)
                ...
                : (function (clone, numbers) {
                    if (
                      numbers.length > 2 ||
                      numbers.length < 1 ||
                      _.filter(numbers, _.pipe(_.identity, isNaN))
                        .length
                    )
                      return _.Err("[] sel in [num] or [num ~ num]");
                    var s = numbers[0],
    ...
    },
    {
      set: function (start, sel, val) {
        ...
        _set(
          _arr.length == 1 ? im.start : im.selected,
          _arr[last],
          val
        );
        return im.start;
      ...
      unshift: function (start, sel) {
        ...
        _unshift.apply(
          null,
          [im.selected].concat(_.rest(arguments, 2))
        );
        return im.start;
      },
    }
    ...
    function make_sel(el) {
      return _.isString(el)
        ? (function (el) {
            ...
            return make_sel(
              document.querySelector('[_sel="' + el + '"]')
            );
          })(el)
        : (function (el) {
            try {
  ...
  !(function (_, notices) {
    ...
    function emitAll(name1, emit_args) {
      ...
      if (_notice)
        for (key in _notice) emit_loop(emit_args, _notice, key);
    }
    function emit(name, keys, emit_args) {
      !(function (_notice, keys) {
        ...
        notices[name],
        _.is_fn(keys) ? keys(_.keys(notices[name])) : keys
      );
    }
  ...
  function Ltake1(lazy, iter) {
    return L.take(
      Ladd(
        lazy && lazy._p_lz && lazy.is_strict ? lazy.data : lazy,
        iter
      ),
      1
    );
